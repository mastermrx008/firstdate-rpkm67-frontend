...
import { HeadingProps, LiProps, OrderedListProps } from "react-markdown/lib/ast-to-react";
import { ReactMarkdownProps } from "react-markdown/lib/complex-types";
import "@/components/congratulation/style.css"

const StyledH1: React.FC<HeadingProps> = ({ children, level, ...props }) => {
    return (
        <h1 className="text-center font-medium whitespace-pre-wrap" {...props}>
            {children}
        </h1>
    );
};
...
const StyledP: React.FC<ReactMarkdownProps> = ({ children, ...props }) => {
    return (
        <p className="text-xs whitespace-pre-wrap" {...props}>
            {children}
        </p>
    );
};

const StyledLI: React.FC<LiProps> = ({ children, index, ...props }) => {
    return (
        <li className="text-xs whitespace-pre-wrap" {...props}>
            {children}
        </li>
    );
};
...
const StyledOL: React.FC<OrderedListProps> = ({ children, ...props }) => {
    return (
        <ol className="flex flex-col pl-4 gap-3" style={{listStyleType: 'decimal'}} {...props}>
            {children}
        </ol>
    );
};

const StyledHR: React.FC<ReactMarkdownProps> = ({children , ...props}) => {
  return (
    <hr className="border border-project-dark-gray" {...props} />
  );
};
